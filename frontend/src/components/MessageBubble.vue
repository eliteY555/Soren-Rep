...
// Configure marked — use default renderer, only override code highlighting
marked.setOptions({
  breaks: true,
  gfm: true
})

// Custom code renderer ONLY (default renderer for everything else)
// Use marked extensions API for v12 compatibility
const codeExtension = {
  name: 'codeBlock',
  level: 'block',
  renderer(token) {
    ...
    return `<div class="code-block">${langLabel}<button class="code-copy" data-code="${escapedCode}">
...
/* Code block wrapper — styled via extension */
